...
fetchData <- function(mysqlPool, psqlPool, shouldFetchAll, startDate, endDate) {
  dbOutput <- list()
  variablesToFetch <- list("BMI","BMI Status","FBS",
                           "Hb1AC")

  diabeticConceptNames <- c("Diabetes, Type 1","Diabetes, Type 2",
   "Diabetes I, uncomplicated", "Diabetes, Juvenile", "Diabetes II, uncomplicated")
  
  diabeticConcepts <- mysqlPool %>%
     tbl("concept_name") %>%
     filter(voided == 0, name %in% diabeticConceptNames,
  ...
  if(shouldFetchAll){
    patientWithDiabetes <- mysqlPool %>% 
      tbl("obs") %>% 
      filter(voided==0,
       value_coded %in% diabeticConcepts,
  ...
  if(nrow(patientWithDiabetes) <= 0){
    return (data.frame())
  }
  encIds <- pull(patientWithDiabetes, encounter_id)
  personIds <- pull(patientWithDiabetes, person_id)

  diabeticVisits <- mysqlPool %>%
    tbl("encounter") %>%
    filter(voided==0,
  ...
  visitIds <- pull(diabeticVisits, visit_id)
  ...
  encounterForDiabeticVisits <- mysqlPool %>%
    tbl("encounter") %>%
    filter(voided==0, visit_id %in% visitIds) %>% 
    select(visit_id,uuid,patient_id) %>%
    collect(n=Inf)

  encounterUUIDs <- pull(encounterForDiabeticVisits, uuid)[]
  ...
  visitPayments <- encounterForDiabeticVisits %>%
  ...
  patients <- patients %>%
    ...
    inner_join(diabeticVisits, by = c("person_id"="patient_id")) %>%
    collect(n=Inf)

  allObsForDiaeticPatients <- mysqlPool %>%
    tbl("obs") %>%
    filter(voided==0, person_id %in% personIds) %>%
    collect(n=Inf)
  ...
  obsForVariables <- allObsForDiaeticPatients %>%
    ...
    filter(!is.na(value_numeric) | !is.na(value_text)) %>%
    ungroup() %>%
